  ...
  const filtered = useMemo(() => {
    ...
    return DATA.filter(item => {
      ...
      return matchCat && (
        ...
        Object.keys(item.glossary).some(k => k.toLowerCase().includes(q)) ||
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .hobby-card:hover { border-color: #3a3a5c; }
        .hobby-card.open { border-color: #5a5aff; }
        .hobby-header {
          display: flex;
          align-items: center;
          gap: 12px;
        ...
        .hobby-icon { font-size: 22px; width: 32px; text-align: center; }
        .hobby-name { font-size: 15px; font-weight: bold; flex: 1; letter-spacing: 0.5px; }
        .hobby-cat {
          font-size: 10px;
          color: #5a5a8a;
        ...
        .chevron { color: #5a5aff; font-size: 12px; transition: transform 0.2s; }
        .chevron.open { transform: rotate(180deg); }
        .tabs { display: flex; border-top: 1px solid #1e1e2e; }
        .tab-btn {
          flex: 1;
          padding: 8px;
          background: none;
        ...
        .tab-btn.active { color: #5a5aff; border-bottom-color: #5a5aff; background: #0d0d1a; }
        .term-row { padding: 8px 16px; border-bottom: 1px solid #111; }
        ...
        .search-bar:focus { border-color: #5a5aff; }
        .cat-btn {
          background: none;
          border: 1px solid #2a2a4a;
          border-radius: 2px;
        ...
        .cat-btn.active { background: #5a5aff; border-color: #5a5aff; color: #fff; }
        .count-badge {
          background: #1e1e3a;
      ...
      {/* Header */}
      <div style={{ borderBottom: "1px solid #1e1e2e", padding: "24px 20px 20px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 4 }}>
            <h1 style={{ margin: 0, fontSize: 28, fontWeight: "bold", letterSpacing: 2, color: "#fff" }}>
              HUMAN ACTIVITY ATLAS
            </h1>
            <span className="count-badge">{DATA.length} HOBBIES</span>
          </div>
          <p style={{ margin: "0 0 16px", color: "#5a5a8a", fontSize: 12, letterSpacing: 1 }}>
            THE COMPLETE SPECTRUM OF HUMAN PURSUITS — TOOLS, GLOSSARY & TECHNIQUES
          </p>
          <input
            className="search-bar"
            placeholder="Search any hobby, tool, or term..."
            value={search}
      ...
      {/* Category filters */}
      <div style={{ borderBottom: "1px solid #1e1e2e", padding: "12px 20px", overflowX: "auto" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", gap: 6, flexWrap: "nowrap" }}>
          <button className={`cat-btn ${selectedCat === "All" ? "active" : ""}`} onClick={() => setSelectedCat("All")}>All</button>
          {CATEGORIES.map(c => (
      ...
      {/* Stats bar */}
      <div style={{ padding: "8px 20px", borderBottom: "1px solid #111", background: "#0d0d15" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", fontSize: 11, color: "#3a3a5a", letterSpacing: 1 }}>
          SHOWING {filtered.length} OF {DATA.length} ACTIVITIES
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "16px 20px" }}>
        {filtered.map(item => {
          ...
          return (
            <div key={item.name} className={`hobby-card ${isOpen ? "open" : ""}`}>
              <div className="hobby-header" onClick={() => {
                setExpanded(isOpen ? null : item.name);
                setTab("glossary");
              }}>
                <span className="hobby-icon">{item.icon}</span>
                <span className="hobby-name">{item.name}</span>
                <span className="hobby-cat">{item.category}</span>
                <span style={{ fontSize: 10, color: "#3a3a5a" }}>{glossaryEntries.length} terms · {item.tools.length} tools</span>
              ...
              {isOpen && (
                <>
                  <div className="tabs">
                    <button className={`tab-btn ${tab === "glossary" ? "active" : ""}`} onClick={() => setTab("glossary")}>
                    ...
                    <div style={{ maxHeight: 400, overflowY: "auto" }}>
                      {glossaryEntries.map(([term, def]) => (
                        <div className="term-row" key={term}>
                          <div style={{ fontWeight: 600, fontSize: 13, color: "#8a8aff", marginBottom: 2 }}>{term}</div>
                          <div style={{ fontSize: 12, color: "#7a7a9a", lineHeight: 1.6 }}>{def}</div>
                        </div>
                      ))}
                    </div>
                  )}
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: "center", padding: 60, color: "#3a3a5a" }}>
            ...
            <div style={{ fontSize: 14, letterSpacing: 1 }}>NO RESULTS FOR "{search.toUpperCase()}"</div>
          </div>
        )}
      </div>
    </div>
